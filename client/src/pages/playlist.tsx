...
export default function Playlist() {
  ...
  const handlePlayPlaylist = async () => {
    console.log('handlePlayPlaylist', playlist, affirmations);
    if (!playlist) return;

    try {
      const res = await fetch(`/api/affirmations?playlistId=${playlist.id}`);
      ...
      console.log('handlePlayPlaylist 2', playlist, data);
      playPlaylist(playlist, data);
    } catch (err) {
  ...
  const handlePlayPause = () => {
    console.log('handlePlayPause', currentTrack, Number(id));
  ...
  const handleSelectBackgroundMusic = (musicId: string) => {
    ...
    console.log('handleSelectBackgroundMusic', selectedMusic);
    if (selectedMusic) {
              ...
              >
                ...
                <div>
                  <Button
                    ...
                    className="h-8 w-8"
                    onClick={() => {
                      console.log('handlePlayPause 2', currentTrack?.affirmations[index].audioUrl, Number(id), index);
                      if (!currentTrack || currentTrack.playlist.id !== Number(id)) {
                        handlePlayPlaylist();
                        console.log('here 1');
                      } else if (currentTrack.currentAffirmationIndex === index) {
                        const audioTrack = currentTrack?.affirmations[index].audioUrl;
                        audioRef.current = new Audio('/audio/confidence-1.mp3');
